...
def load_and_validate(file_path: str | Path):
    ...
    # CANAL se convierte a categórico AQUÍ, explícitamente, con un conjunto
    # de categorías fijo y ordenado alfabéticamente (una única pasada con
    # `.unique().compute()`), en lugar de pasar `dtype="category"` a
    # `dd.read_csv` (como se hacía antes).
    #
    # Motivo (bug de no-reproducibilidad detectado y corregido):
    #   Con `dtype="category"` en `read_csv`, Dask infiere las categorías
    #   de forma INDEPENDIENTE en cada una de las particiones (792 en este
    #   dataset), a partir únicamente de los valores presentes en esa
    #   partición. El dtype categórico queda "unknown" hasta que se
    #   unifica -- típicamente al hacer `.compute()` -- y el orden final
    #   de `.cat.categories` depende del orden en que las particiones se
    #   concatenan/procesan, que con el planificador multi-hilo de Dask
    #   puede variar entre corridas (mismo seed, mismo código).
    #   Esa inestabilidad se propagaba silenciosamente a
    #   `pd.get_dummies(..., drop_first=True)` en
    #   `src/modeling_regression.py`: la categoría "de referencia" (la que
    #   `drop_first` descarta) cambiaba de una corrida a otra, alterando
    ...
    logger.info(
        ...
        "(confirmado empíricamente sobre el archivo real, ej. '1999-04-01') "
