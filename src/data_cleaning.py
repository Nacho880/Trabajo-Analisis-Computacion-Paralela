...
memoria por `main.py` vía `.persist()`, pero aún como colección Dask
...
Diseño: Dask de punta a punta en esta etapa, no Pandas.
    ...
    Rendimiento: `main.py` llama a `.persist()` sobre el DataFrame
    crudo inmediatamente después de `load_and_validate(...)`, ANTES de
    invocar `clean_dataset`. Esto es lo que hace viable tener múltiples
...
Todas las operaciones que involucran aleatoriedad (imputación
estocástica) usan `config.SEED` explícitamente, cumpliendo el
requisito de reproducibilidad total.
"""
...
def clean_dataset(df: dd.DataFrame) -> tuple[dd.DataFrame, CleaningReport]:
    ...
    en memoria por `main.py`). La materialización final a Pandas
    (`.compute()`) ocurre en `main.py`, después de que también
    ...
    Complejidad:
        ...
        perezoso.
    """
    logger.info("Semilla de reproducibilidad activa para este pipeline: %d", SEED)
